...
(define interpret-statement-list
  (lambda (statement-list environment return break continue throw)
    (if (null? statement-list)
        (run-main environment return break continue throw)
...
; looks for 'main method and runs function
(define run-main
  (lambda (environment return break continue throw)
    (cond
      [(not (exists? main environment)) (myerror "main function not defined")]
      [else                             (interpret-statement-list (get-func-body (lookup main environment))
...
(define interpret-statement
  (lambda (statement environment return break continue throw)
    (cond
      ((eq? 'funcall (statement-type statement))  (interpret-funcall-runner
                                                   (get-func-body (lookup (get-name statement) environment))
                                                   (bind-value-list
                                                    (get-name-list (lookup (get-name statement) environment))
                                                    (get-value-list statement) (push-frame environment) throw)
                                                   return break continue throw))
      ((eq? 'function (statement-type statement)) (interpret-function
                                                   (body statement)
                                                   environment return break continue throw))
                ...
                (call/cc
                 (lambda (break)
                   (interpret-statement (func-body statement-list)
                    environment break break continue throw))))
           (interpret-funcall-runner (body statement-list)
                (call/cc
                 (lambda (break)
                   (interpret-statement (func-body statement-list)
  ...
  (lambda (funcall environment throw)
    (call/cc
     (lambda (func-return)
       (cond
         [(not (exists?
          (get-func-name funcall) environment)) (myerror "error: function name not declared:"
                                                         (get-func-name funcall))]
         [(null? (get-param-name funcall))      (interpret-function-runner
                                                 (get-func-body (lookup (get-func-name funcall) environment))
                                                 (push-frame (pop-frame environment)) func-return
                                                 (lambda (env) (myerror "Break used outside loop"))
                                                 (lambda (env) (myerror "Continue used outside of loop"))
                                                 throw)]
         [else                                  (interpret-function-runner
                                                 (get-func-body (lookup (get-func-name funcall) environment))
                                                 (bind-value-list
                                                  (get-name-list (lookup (get-func-name funcall) environment))
                                                  (get-param-name funcall)(push-frame environment) throw)
                                                 func-return
                                                 (lambda (env) (myerror "Break used outside loop"))
                                                 (lambda (env) (myerror "Continue used outside of loop"))
                                                 throw)])))))

; Helper method that evaluates the expressions within the function
(define interpret-function-runner
  (lambda (statement environment return break continue throw)
    (cond
      [(null? statement) (pop-frame environment)]
      [else              (interpret-function-runner
                          (body statement)
                          (interpret-statement
                           (get-statement statement) environment return break continue throw)
                          return break continue throw)])))

; Calls a function that is nested within a function body
     ...
     (lambda (break)
       (letrec ((loop (lambda (condition body environment)
                        (if (eval-expression condition environment throw)
                            (loop condition body
                                  (interpret-statement
                                   body environment return break
                                   (lambda (env) (break (loop condition body env))) throw))
                         environment))))
         (loop (get-condition statement) (get-body statement) environment))))))

; Helper function that evaluates statements within the block
(define interpret-block-runner
  (lambda (statement environment return break continue throw)
    (cond
      [(null? statement) environment]
      [else              (interpret-block-runner
                          (body statement)
                          (interpret-statement (car statement) environment return break continue throw)
                          return break continue throw)])))

; Interprets a block.  The break, continue, and throw continuations must be adjusted to pop the environment
(define interpret-block
  (lambda (statement environment return break continue throw)
    (pop-frame (interpret-block-runner (body statement)
...
(define eval-operator
  (lambda (expr environment throw)
    (cond
      ...
      (else
       (eval-binary-op2 expr (eval-expression (operand1 expr) environment throw) environment throw)))))
...
(define body cdr)
(define get-statement car)
(define get-name-list operator)
(define get-value-list cddr)
(define get-name operand1)
(define get-param-name cdr)
(define get-func-name operator)
(define get-func-body operand1)
(define func-body operator)
...
; Binds the parameter names and values to a new layer in the environment
(define bind-value-list
  (lambda (name-list value-list environment throw)
    (cond
      [(not (eq? (length name-list) (length value-list))) (myerror "error: wrong number of param arguments")]
      [(null? name-list)                                  environment]
      [(list? name-list)                                  (bind-value-list
                                                           (get-tail name-list)
                                                           (get-tail value-list)
                                                           (insert
                                                            (get-head name-list)
      ...
      [else                                               (insert
                                                           name-list
                                                           (eval-expression
                                                            value-list (pop-frame environment))
                                                           environment)])))
